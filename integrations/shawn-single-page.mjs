...
function buildExecutionPrompt(compiled) {
  ...
  return [
    ...
    "For the three Directors, create exactly the three skill-defined roles and immediately trigger each role's assigned prompt and non-overlapping output task. Await their required files and terminal status before prepare-directors. Once all three required Director files exist and validate, that canonical file evidence is authoritative: stop any extra Director waiting and run prepare-directors immediately in this same turn without asking for a user steer. Never leave a spawned Director idle, and never keep the root turn parked on a completed Director UI status.",
    ...
    "Pass runtime.request_started_at to the preflight manifest and MUST pass outline_path as both required-file and page-source. The preflight builder and initializer deterministically recognize the canonical outline's slide_identity_required declaration and bind that same outline; identity binding is not an optional model decision. You may also pass the same outline as slide-identity-file only to build_fast8_preflight_manifest.py, but never pass --slide-identity-file to init_task_dir.py for Fast8. Initialization must fail before ImageGen if identity.page_id cannot be projected to identity.slide_uid. Pass runtime.overview_python as overview Python and runtime.monitoring_root as the existing monitoring root. Do not create a second UID file, monitor, semaphore, Judge, reviewer, state machine, or outline copy.",
    "For preflight asset_items, the first user-designated style image must use role=primary_style_reference and any additional style images must use role=supporting_style_reference. style_anchor_only is an approval scope, never an asset role. A role mismatch is a hard preflight failure: do not patch the frozen manifest or canonical state by hand.",
