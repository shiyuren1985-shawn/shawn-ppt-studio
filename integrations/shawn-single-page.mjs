...
function buildExecutionPrompt(compiled) {
  ...
  return [
    ...
    "Pass runtime.request_started_at to the preflight manifest and outline_path as required-file plus page-source. If the canonical outline itself must be registered for UID binding, pass that exact same outline as slide-identity-file only to build_fast8_preflight_manifest.py. Never pass --slide-identity-file to init_task_dir.py for Fast8; it reads the optional binding from runtime.preflight_path. Pass runtime.overview_python as overview Python and runtime.monitoring_root as the existing monitoring root. Do not create a second UID file, monitor, semaphore, Judge, reviewer, state machine, or outline copy.",
