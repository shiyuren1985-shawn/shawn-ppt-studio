...
内容合同含 `language_presentation` 时仍使用同一个内容门：`same_page` 的英文须紧邻对应中文并共享同一信息单元；`split_peer` 的中文页／英文页各自只检查本页语言，不要求同图双语，也不得出现兄弟页另一语言的新增正文。兄弟页是否齐全由控制面机械校验。明显缺失同页配对英文，或把英文集中成独立底栏、侧栏、第二套版式，判为内容失败。正式品牌名、产品名和标准名除外。不增加 OCR、逐页放大或第二个 Reviewer。
