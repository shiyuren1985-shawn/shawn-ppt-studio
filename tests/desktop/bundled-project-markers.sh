...
test -f "$RESOURCES/server/conversation-file.mjs"
test -f "$RESOURCES/server/conversation-image.mjs"
test -f "$RESOURCES/assets/Public_Label_Template.pptx"
...
grep -q 'requestUrl.pathname === "/api/exports/open-folder"' "$RESOURCES/server/http-server.mjs"
grep -q 'verifyPreservedPublicLabel' "$RESOURCES/server/server.mjs"
grep -q '打开导出文件夹' "$RESOURCES/web/selector/workspace.js"
