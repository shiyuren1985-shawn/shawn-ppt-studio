...
test("a legacy selector project requires explicit Studio migration", async () => {
  const discovery = {
    async readDeck() {
      return {
        deck_id: "demo",
  ...
  await assert.rejects(workspace.refresh("demo"), (error) => {
    assert.equal(error.code, "legacy_project_migration_required");
